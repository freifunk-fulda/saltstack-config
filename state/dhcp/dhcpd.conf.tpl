...
subnet 10.185.0.0 netmask 255.255.192.0 {
        ...
        option routers 10.185.0.{{ pillar['hosts'][grains['id']]['id'] }}{% for host in pillar['hosts'].keys() if host != grains['id'] -%}, 10.185.0.{{ pillar['hosts'][host]['id'] }}{% endfor -%};
        option domain-name-servers 10.185.0.{{ pillar['hosts'][grains['id']]['id'] }}{% for host in pillar['hosts'].keys() if host != grains['id'] -%}, 10.185.0.{{ pillar['hosts'][host]['id'] }}{% endfor -%};
        option ntp-servers ntp{{ pillar['hosts'][grains['id']]['id'] }}.services.fffd{% for host in pillar['hosts'].keys() if host != grains['id'] -%}, ntp{{ pillar['hosts'][host]['id'] }}.services.fffd{% endfor -%};
}
